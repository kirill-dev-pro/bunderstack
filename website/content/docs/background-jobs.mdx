...
Fixtures never auto-start background work. `t.jobs.tick()` claims the runnable
snapshot that existed at the start of the tick and returns what it did:

```ts
await using t = await backend.test()
const { claimed, ran, failed } = await t.jobs.tick()
```

Jobs recursively enqueued by handlers wait for the next tick. `settle()` repeats
snapshots until the queue converges, a terminal job fails, or configured limits
are reached; delayed jobs are not made runnable by implicit clock advancement.
