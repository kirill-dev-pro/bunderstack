...
function resolvedAccess(overrides: Parameters<typeof validateAndResolveAccess>[1] = {}) {
...
describe('sanitizeWriteBody', () => {
  it('create with no writableColumns passes id through', () => {
    ...
    const out = sanitizeWriteBody({ id: 'x', title: 't' }, access, 'create', null)
  ...
  it('update strips id (id is immutable on update)', () => {
    const access = resolvedAccess()
    const out = sanitizeWriteBody({ id: 'x', title: 't' }, access, 'update', null)
    expect(out.id).toBeUndefined()
    expect(out.title).toBe('t')
  })

  it('create with explicit writableColumns excluding id does not pass id through', () => {
    const access = resolvedAccess({ posts: {
      list: 'public',
      get: 'public',
      create: 'public',
      update: 'public',
