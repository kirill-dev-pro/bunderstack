// src/crud.pg.test.ts — CRUD surface parity on PGlite. Mirrors crud.test.ts
// exactly (same assertions/response shapes); only the table + db are pg.
...
test('GET /api/posts?q= search is case-insensitive on Postgres', async () => {
  const res = await app.request('/api/posts?q=uniquesearchterm')
  expect(res.status).toBe(200)
  const body = (await res.json()) as { items: { title: string }[] }
  expect(body.items.some((p) => p.title === 'UniqueSearchTerm')).toBe(true)
})
...
test('GET /api/posts defaults to stable sort order', async () => {
  ...
  expect(res.status).toBe(200)
  const body = (await res.json()) as {
    items: { id: number; createdAt: string | number }[]
    sort: string
    order: string
  }
