...
plus an `env` schema when the application declares one. The callback receives
the validated environment and returns everything that depends on it. It is
pure: nothing connects until `backend.start()`.
