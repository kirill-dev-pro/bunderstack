...
function createBunderstack<
  ...
  TJobs,
  TApi,
>(options: BunderstackConfig<...>): Promise<BunderstackApp<...>>
```
...
- `api: (o) => router`: custom oRPC procedures;
...
```ts
api: (o) => ({
  publicProcedure: o.public,
  protectedProcedure: o.protected,
  providerWebhook: o.webhook,
})
```

All three are oRPC procedure builders with the shared Bunderstack error
...
`bunderstack/database/*` subpath. The schema dialect and adapter dialect must
match.
...
The root package exports `mockAuthSession` for scoped session tests. Disable
the automatic worker with `background: { autoStart: false }`, then drive
`app.jobs.tick()` directly for deterministic job tests.
