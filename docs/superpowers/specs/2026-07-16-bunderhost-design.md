...
- Multi-bucket приложения: как платформа узнаёт список декларированных
  бакетов (кандидат — introspection через `schema-export`-подобный
  механизм или конвенция «один бакет в v1»).
