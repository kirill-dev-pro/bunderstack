...
`bunderstack({ schema, env }, (env) => config)` only declares the backend. The
first argument is the static half — the Drizzle schema and the environment
schema — and the callback returns everything that depends on a validated value.
The callback runs once per operation, never at import time.
