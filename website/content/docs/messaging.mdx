...
A builder declared inline in the config callback receives an open messaging
type, because TypeScript cannot infer the channel record and type the callback
in the same pass. Declare the builder in its own module to get the exact type:
