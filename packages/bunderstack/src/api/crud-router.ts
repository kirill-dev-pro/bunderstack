...
> = {
  [K in keyof TSchema as TSchema[K] extends Table ? K : never]: TableCrudProcedures
}
...
>(
  ...
  const routerObj: Record<string, TableCrudProcedures> = {}
  ...
  for (const [tableKey, table] of Object.entries(schema)) {
    ...
    const updateShape = { ...insertSchema.partial().shape }
    delete (updateShape as any).id
    ...
    // 3. CREATE procedure
    const createProc = builder.public
      .meta(
        openapi({
          method: 'POST',
          path: `/api/${name}`,
  ...
  return routerObj as CrudApiRouterFor<TSchema>
}
