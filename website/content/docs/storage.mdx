...
Use `app.storage.upload` (or `ctx.storage.upload` in background jobs and tRPC procedures) to upload server-generated files (e.g. generated PDFs or exported reports). It uploads the bytes to storage and automatically registers a `file_meta` record so the file can be retrieved via `GET /api/files/<key>`:
